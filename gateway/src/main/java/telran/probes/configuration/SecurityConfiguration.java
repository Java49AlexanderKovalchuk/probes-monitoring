...
public class SecurityConfiguration {
	...
	SecurityFilterChain configure(HttpSecurity http) throws Exception {
		...
		http.authorizeHttpRequests(requests -> requests.requestMatchers(UrlConstants.UPDATE_EMAILS)
				.hasRole(emailsAdminRole).requestMatchers(UrlConstants.UPDATE_RANGE).hasRole(rangesAdminRole)
				.requestMatchers(rangeSensorUrl + "/**").hasRole(rangesUserRole)
				.requestMatchers(emailsSensorUrl + "/**").hasRole(emailsUserRole).requestMatchers(accountsUrl + "/**")
				.hasRole(accountsUserRole));
