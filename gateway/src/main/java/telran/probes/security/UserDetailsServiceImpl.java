...
public class UserDetailsServiceImpl implements UserDetailsService {
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		String[] roles = Arrays.stream(account.roles()).map(role -> "ROLE_" + role).toArray(String[]::new);
		log.debug("username: {}, password: {}, roles: {}", account.email(), account.password(),
	...
	private AccountDto getAccount(String username) {
		...
		try {
			...
			log.error("no account provided for user {}, reason: {}", username, e.getMessage());
	...
	private String getFullUrl(String username) {
		String res = String.format("http://%s:%d%s/%s", host, port, url, username);
		log.debug("url:{}", res);
		return res;
	}
